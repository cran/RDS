...
#' Writes out the RDS tree in Gephi format
#' @param x An rds.data.frame.
#' @param file a character vector representing a file.
#' @param scale a positive scaler to scale the outputed positions. 
#' @details All the variables in the RDS data set are exported as well.
#' \code{paste0(file,".GEXF")} contains the nodal attributes and edge information.
#' @export
write.gephi <- function(x, file=NULL, scale=20){
	if(is.null(file)){
	  file<-paste(paste(getwd(),substitute(x),sep="/"),"gexf",sep=".")
	}
	x <- as.rds.data.frame(x)
	id <- as.char(get.id(x))
	rid <- as.char(get.rid(x))
	...
	nVizAtt <- t(col2rgb((rid==0)+1, alpha=TRUE))
	nVizAtt <- nVizAtt / max(nVizAtt,na.rm=TRUE)
        t <- reingold.tilford.plot(x, plot=FALSE)
	nVizAtt <- list(position=data.frame(scale*t,0), color=nVizAtt)
